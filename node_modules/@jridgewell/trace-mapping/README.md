...
You may already be familiar with the [`source-map`](https://www.npmjs.com/package/source-map) package's `SourceMapConsumer`. This provides the same `originalPositionFor` and `generatedPositionFor` API, without requiring WASM.
